...
- $S(t)$ - cena rynkowa towaru będącego przedmiotem umowy w momencie $t=1$.
...
Wygenerujmy teraz wektor losowy $\mathbf Z^{(i)} = (\mathbf Z\ |\ \mathbf Z\in W_i')$, którą potem będziemy mogli przekształcić na  zmienną z wybranej warstwy $(\mathbf B\ |\ \mathbf B\in W_i) = A\mathbf Z^{(i)}$. 
...
Następnie wylosujmy wartość $D_i$ - odległość $\mathbf Z^{(i)}$ od $\mathbf 0$.
...
$$D_i=\sqrt{F^{-1}_{\chi^2_n} \left( \frac{i-1}{m} +\frac{U}{m} \right)},$$
gdzie $F^{-1}_{\chi^2_n}$ jest dystrybuantą odwrotną rozkładu $\chi^2$  z $n$ stopniami swobody. Ostatecznie otrzymujemy $\mathbf Z^{(i)}=D_i\cdot\frac{\boldsymbol \xi}{||\boldsymbol \xi||}$.
